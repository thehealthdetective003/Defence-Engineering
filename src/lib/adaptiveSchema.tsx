...
const hasFacilityContract = (data: any) =>
  String(data?.schema_version || '').includes('defence_facility_visual_handoff') ||
  !!data?.facility_identity_lock ||
...
export function buildFacilityIdentitySummary(topic?: TopicBrief | null): string {
  const lock = topic?.facility_identity_lock;
  if (!lock) return '';
  return [
    lock.core_geometry,
    lock.surface_finish,
...
export function validateAdaptiveBrief(data: any): string[] {
  ...
  if (!normalized.topic?.facility) missing.push('topic.facility');
  ...
  if (hasFacilityContract(data) && getNegativePromptGlobal(normalized).length === 0) missing.push('negative_prompt_global');
  return missing;
}
...
export function validateAdaptiveWarnings(data: any): string[] {
  ...
  const facilityContract = hasFacilityContract(data);
  ...
  if (facilityContract && (count < 5 || count > 8)) warnings.push(`lifecycle_stage_count should resolve to 5-8; currently ${count || 0}`);
  if (!topic.facility_identity_lock) warnings.push('facility_identity_lock missing');
  ...
  getLifecycleStages(topic).forEach((stage, index) => {
    ...
    if (facilityContract && (!stage.primary_camera_shot || !stage.motion_direction || !stage.quality_control_focus || !stage.visual_risk_notes)) {
  ...
  const truthText = JSON.stringify({
    topic: topic.topic,
    facility_identity_lock: topic.facility_identity_lock,
