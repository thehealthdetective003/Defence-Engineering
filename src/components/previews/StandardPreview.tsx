...
export const StandardPreview: React.FC<StandardPreviewProps> = ({ data }) => {
  ...
  const handoff=data._production_handoff;
  const facilityStatus=handoff?.facility.facility_status||data.topic?.facility_status||'UNKNOWN';
  const layoutTruth=handoff?.site_dimensions_and_spatial_relations.layout_claim_status||'UNKNOWN';
  const siteClaims=handoff?.visual_story_plan.chapters.flatMap(chapter=>chapter.visual_beats.map(beat=>beat.facility_claim_status))||[];
  const siteTruth=siteClaims.includes('EXACT_SITE_VERIFIED')?'EXACT SITE VERIFIED':siteClaims.includes('FACILITY_TYPE_CORROBORATED')?'FACILITY TYPE CORROBORATED':siteClaims.includes('CONTEXTUAL_DEFENCE_INFRASTRUCTURE')?'CONTEXTUAL INFRASTRUCTURE':'GENERIC / NON-IDENTIFYING';
    ...
    <div className="space-y-6 text-foreground">
      <div className="border-b border-border/20 pb-4">
        <div className="flex flex-wrap gap-2 mb-3">
          ...
          <Badge variant="outline" className="font-mono text-[10px]">Facility Status · {facilityStatus.replaceAll('_',' ')}</Badge>
          <Badge variant="outline" className="font-mono text-[10px]">Site Truth · {siteTruth}</Badge>
          <Badge variant="outline" className="font-mono text-[10px]">Layout Truth · {layoutTruth.replaceAll('_',' ')}</Badge>
          {facilityStatus==='ACTIVE_PUBLICLY_DOCUMENTED'&&<Badge variant="outline" className="font-mono text-[10px] text-red-400 border-red-500/40">Sensitivity Restrictions Active</Badge>}
        </div>
        ...
        <div className="flex flex-wrap gap-x-4 gap-y-1 text-sm text-muted-foreground mt-1">
          <p>Facility: <span className="text-foreground font-medium">{data.topic?.facility || data.topic?.title || 'Unknown'}</span></p>
          <p>Category: <span className="text-foreground font-medium">{data.topic?.category || 'N/A'}</span></p>
          <p>Country / operator: <span className="text-foreground font-medium">{data.topic?.country_or_operator || 'Unknown'}</span></p>
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div className="space-y-4">
          <section className="p-4 rounded-lg border border-border/20 bg-muted/20">
            <span className="text-[10px] uppercase tracking-wider text-primary font-mono block mb-1">Facility Identity Lock</span>
            <JsonBlock value={data.facility_identity_lock || 'No facility identity lock specified.'} />
          </section>
