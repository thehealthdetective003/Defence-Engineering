...
const omniSectionsProperties = {
  cinematography:{type:Type.STRING},subject:{type:Type.STRING},action:{type:Type.STRING},environment:{type:Type.STRING},style_lighting:{type:Type.STRING},facility_state:{type:Type.STRING},sound:{type:Type.STRING},exclusions:{type:Type.STRING},
};
...
export function Phase4Visuals({ state, setState }: Props) {
  ...
  const requestPrompts = async (ai: GoogleGenAI, selected: typeof directions): Promise<T2VPrompt[]> => {
    ...
    const response = await ai.models.generateContent({ model: settings.model, contents: JSON.stringify(buildFlowContext(state.topic, selected, state.t2vPromptProfile)), config: {
      ...
      systemInstruction:omni
        ? `Return one structured facility-documentary direction for every supplied scene and copy each scene number exactly. Fill prompt_sections with complete natural-language clauses for cinematography, subject, visible construction action, environment, style_lighting, facility_state, synchronized sound, and exclusions. The supplied facility handoff, construction stage, A/B/C state, facility visibility, facility/site claim status, layout claim status, generation permission, media routes, references, required and forbidden features, truth constraints, continuity, and graphic_spec are authoritative. Preserve permanent works already present, absent not-yet-built works, temporary works, exposed geology, excavated or open sections, spoil state, lining/support state, and installed infrastructure. Never auto-complete a facility. Keep drilling, blasting, excavation, spoil handling, tunnelling, cranes, concrete, lining, workers, machinery, geology, groundwater, and internal systems physically plausible. Construction blasting is historical engineering work, never combat. Sound must correspond only to visible or physically present sources. REFERENCE_MEDIA and archive/authentic routes must use cited evidence and must never turn generated reconstruction into authentic footage. Satellite context must not reveal access or security guidance. Graphics must obey layout truth: exact only from verified references, partial only within documented bounds, conceptual explicitly non-to-scale, and unknown with no internal cutaway or floor plan. Never invent room arrangements, hidden entrances, current access routes, guards or patrols, security procedures, surveillance positions or blind spots, vulnerabilities, sabotage routes, military signage, blast/survivability claims, readable fake labels, exact-site identity, combat, or weapon discharge. Do not return voiceover, duration, headings, or raw JSON text.`
        : `Create direct facility-construction documentary video prompts only. Return exactly one item per supplied scene and copy its number. Do not return or rewrite voiceover. Use the supplied facility subject, construction action, environment, camera, materials, chronology, truth fields, media route, continuity, and visible/absent features. ${profileInstruction(state.t2vPromptProfile)}`,
    }});
  ...
  const exportCsv = () => download('t2v-prompts.csv', ['profile,number,start,end,duration,chapter,beat,visual_family,story_function,visual_treatment,facility_visibility,facility_claim_status,layout_claim_status,generation_permission,energy_level,graphic_subtype,graphic_claim,graphic_composition,graphic_motion,stage,state,action,voiceover,t2v_prompt,keywords,continuity,quality_flags',...state.visualPrompts.map(p=>{const d=directions[p.number-1];return [state.t2vPromptProfile,p.number,d?.start,d?.end,d?.duration,d?.chapter_id,d?.beat_id,d?.visual_family,d?.story_function,d?.visual_treatment,d?.facility_visibility,d?.facility_claim_status,d?.layout_claim_status,d?.generation_permission,d?.energy_level,d?.graphic_spec?.graphic_subtype,d?.graphic_spec?.visual_claim,d?.graphic_spec?.composition,d?.graphic_spec?.motion_pattern,p.stage_id,p.state,p.action_description,p.voiceover,p.video_prompt,p.stock_keywords,p.continuity_notes,(p.quality_flags||[]).join('|')].map(csvCell).join(',')})].join('\n'),'text/csv');
  ...
  return <div className="space-y-6">
    <Button variant="link" className="workspace-back p-0" onClick={()=>setState(s=>({...s,phase:2}))}><ArrowLeft className="mr-1 h-3 w-3"/>Back to scene direction</Button>
    <div className="inset-panel space-y-3 rounded-2xl p-5">
      <label className="section-kicker">Generation target</label>
      <Select value={state.t2vPromptProfile} onValueChange={(value) => setState(previous => ({ ...previous, t2vPromptProfile:value as T2VPromptProfile, visualPrompts:[], demoScenes:[], demoSceneNumbers:[], demoState:'idle' }))}>
        <SelectTrigger className="h-11 max-w-md border-border/65 bg-card/55"><SelectValue/></SelectTrigger><SelectContent><SelectItem value="omni-flash">Gemini Omni Flash</SelectItem><SelectItem value="veo-flow">Veo / Google Flow</SelectItem></SelectContent>
      </Select>
    ...
    {batchProgress && <div className={`space-y-3 rounded-2xl border p-4 ${batchProgress.error?'border-red-500/40 bg-red-500/5':'border-primary/30 bg-primary/5'}`}>
    ...
    <div className="space-y-4">{shown.map(prompt=>{const d=directions[prompt.number-1];return <div key={prompt.number} className="surface-panel space-y-3 rounded-2xl p-4 sm:p-5">
      ...
      <div className="rounded-xl border border-border/40 bg-muted/30 p-3 text-xs leading-5"><div className="mb-1 font-mono text-[9px] uppercase tracking-wider text-muted-foreground">Exact imported VO {d?.silent?'· silent window':''}</div>{prompt.voiceover||'[SILENT]'}</div>
